...
class Simulation():
    ...
    def simulate_combat(self, hunter: Hunter) -> None:
        ...
        if isinstance(hunter, Borge) and hunter.has_athena():
            hpush(self.queue, (round(hunter.athena_speed, 3), 1, 'athena'))
            ...
            while self.enemies:
                ...
                while not enemy.is_dead() and not hunter.is_dead():
                    ...
                    match action:
                        ...
                        case 'athena':
                            hunter.athena_attack(enemy)
                            if isinstance(hunter, Borge) and hunter.has_athena() and not hunter.is_dead():
                                hpush(self.queue, (round(prev_time + hunter.athena_speed, 3), 1, 'athena'))
                        case 'enemy':
                            enemy.attack(hunter)
