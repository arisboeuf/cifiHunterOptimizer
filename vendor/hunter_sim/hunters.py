...
class Hunter:
    ...
    def validate_build(self) -> Tuple[int, int, set, int, int]:
        ...
        for insc in self.inscryptions.keys():
            if insc not in self.costs["inscryptions"]:
                invalid.add(insc)
                continue
            if (lvl := self.inscryptions[insc]) > self.costs["inscryptions"][insc]["max"]:
                invalid.add(insc)
    ...
    def compute_loot(self) -> float:
        ...
        if isinstance(self, Borge):
            ...
            additional_multipliers = (
                (1 + (self.inscryptions.get("i60", 0) * 0.03))
                * (1.1 ** self.inscryptions.get("i14", 0))
                * (1.08 ** self.inscryptions.get("i44", 0))
                * (1.1 ** self.inscryptions.get("i80", 0))
                * (1.08 ** self.inscryptions.get("i103", 0))
            )
...
class Borge(Hunter):
    ...
    costs = {
        ...
        "attributes": {
            ...
            "soul_of_the_minotaur": {  # unique mult: +1% power, +1% DR per level
                "cost": 2,
                "max": 20,
                "unlock_spent": 108,
            },
            "soul_of_hermes": {  # +0.4% crit / effect chance, +1% crit power per level
                "cost": 2,
                "max": 20,
                "unlock_spent": 108,
            },
            "soul_of_athena": {  # +1 heavy attack (x1.5 dmg, x6 charge, always crits)
                "cost": 15,
                "max": 1,
                "unlock_spent": 138,
            },
        ...
        "inscryptions": {
            ...
            "i44": {  # x1.08 loot per level
                "cost": 1,
                "max": 10,
            },
            "i60": {  # +3% ATK / HP / loot per level
                "cost": 1,
                "max": 10,
            },
            "i80": {  # x1.1 loot per level
                "cost": 1,
                "max": 10,
            },
            "i84": {  # +5% Max HP per level
                "cost": 1,
                "max": 10,
            },
            "i87": {  # x1.05 ATK power per level
                "cost": 1,
                "max": 10,
            },
            "i88": {  # +0.4% crit chance
            ...
            "i89": {  # +0.2% effect chance
                "cost": 1,
                "max": 7,
            },
            "i91": {  # +0.2% DMG reduction
                "cost": 1,
                "max": 7,
            },
            "i103": {  # x1.08 loot per level
                "cost": 1,
                "max": 8,
            },
        },
    ...
    def __init__(self, config_dict: Dict):
        ...
        self.total_athena_attacks: int = 0
        self.total_athena_damage: float = 0
    ...
    def __create__(self, config_dict: Dict) -> None:
        ...
        self.max_hp = (
            ...
            * (1 + (self.inscryptions.get("i84", 0) * 0.05))
        ...
        self.power = (
            ...
            * (1 + (self.inscryptions["i60"] * 0.03))
            * (1.05 ** self.inscryptions.get("i87", 0))
            ...
            * (1 + (self.attributes.get("soul_of_the_minotaur", 0) * 0.01))
        )
        # regen
        self.regen = (
            (
        ...
        self.damage_reduction = (
            (
                ...
                + (self.inscryptions.get("i91", 0) * 0.002)
            )
            * (1 + (0.02 * self.gems["creation_node_#2"]))
            * (1 + (self.attributes.get("soul_of_the_minotaur", 0) * 0.01))
        )
        ...
        self.effect_chance = (
            (
                ...
                + (self.inscryptions.get("i89", 0) * 0.002)
                + (0.03 * self.gems["innovation_node_#3"])
                + (self.attributes.get("soul_of_hermes", 0) * 0.004)
            )
            * (1 + (0.02 * self.gems["creation_node_#2"]))
        )
        ...
        self.special_chance = (
            (
                ...
                + (self.inscryptions.get("i88", 0) * 0.004)
                + (self.attributes.get("soul_of_hermes", 0) * 0.004)
            )
            * (1 + (0.02 * self.gems["creation_node_#2"]))
        )
        ...
        self.special_damage = (
            ...
            + (self.attributes.get("soul_of_hermes", 0) * 0.01)
        )
        # speed
        self.speed = (
            5
    ...
    def has_athena(self) -> bool:
        return self.attributes.get("soul_of_athena", 0) > 0

    def athena_attack(self, target) -> None:
        """Soul of Athena heavy attack: always crits for x1.5 damage, charges x6 slower."""
        damage = self.power * 1.5 * self.special_damage
        self.total_crits += 1
        self.total_extra_from_crits += max(0.0, damage - self.power)
        self.total_athena_attacks += 1
        self.total_athena_damage += damage
        logging.debug(
            f"[{self.name:>{hunter_name_spacing}}][@{self.sim.elapsed_time:>5}]:\tATHENA\t{damage:>6.2f} (always crit)"
        )
        if self.mods["trample"] and not target.is_boss() and damage > target.max_hp:
            trample_kills = self.apply_trample(damage, current_target=target)
            if trample_kills > 1:
    ...
    @property
    def athena_speed(self) -> float:
        """Charge time for Athena heavy attack (x6 main attack interval)."""
        return self._speed * 6.0
    ...
    def get_results(self) -> List:
        ...
        return super(Borge, self).get_results() | {
            ...
            'athena_attacks': self.total_athena_attacks,
            'athena_damage': self.total_athena_damage,
        }
